import { Inquiry, InquiryAction, CelebrationInquiryDetails, WeddingInquiryDetails, CorporateInquiryDetails, Invoice, Order, Quote } from '../types';
import { FormCategory } from "../pages/Forms";
...
export const getInvoicesByStatus = (status: string): Invoice[] => {
  return mockInvoicesList.filter(invoice => invoice.status === status);
};

export const getOverdueInvoicesCount = (): number => {
  return mockInvoicesList.filter(invoice => invoice.status === 'overdue').length;
};

export const getPendingInvoicesCount = (): number => {
  return mockInvoicesList.filter(invoice => invoice.status === 'pending' || invoice.status === 'partial').length;
};

export const getInvoiceById = (id: string): Invoice | undefined => {
  return mockInvoicesList.find(invoice => invoice.id === id);
};

export const getInvoicesByCustomer = (email: string): Invoice[] => {
  return mockInvoicesList.filter(invoice => invoice.email.toLowerCase() === email.toLowerCase());
};
...
export const getTotalRevenue = (): number => {
  return mockInvoicesList.reduce((sum, invoice) => sum + invoice.total, 0);
};

export const getTotalOutstanding = (): number => {
  return mockInvoicesList.reduce((sum, invoice) => sum + invoice.balance, 0);
};

// Helper functions for orders
export const getOrdersByStatus = (status: string): MockOrder[] => {
  return mockOrdersList.filter(order => order.status === status);
};

export const getNewOrdersCount = (): number => {
  return mockOrdersList.filter(order => order.status === 'inquiry').length;
};

export const getOrderById = (id: string): MockOrder | undefined => {
  return mockOrdersList.find(order => order.id === id);
};

export const getOrdersByType = (type: string): MockOrder[] => {
  return mockOrdersList.filter(order => order.eventType === type);
};
...
export const getQuotesByStatus = (status: string): MockQuote[] => {
  return mockQuotesList.filter(quote => quote.status === status);
};

export const getNewQuotesCount = (): number => {
  return mockQuotesList.filter(quote => quote.status === 'draft').length;
};

export const getQuoteById = (id: string): MockQuote | undefined => {
  return mockQuotesList.find(quote => quote.id === id);
};

export const getQuotesByType = (type: string): MockQuote[] => {
  return mockQuotesList.filter(quote => quote.eventType === type);
};

export const getRecentQuotes = (limit: number = 5): MockQuote[] => {
