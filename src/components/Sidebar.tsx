...
import { Home, DollarSign, FileText, Receipt, BarChart3, Factory, ChefHat, Package, ShoppingCart, Archive, Users, MessageSquare, MapPin, Ligature as FileSignature, FilePen, Settings, UserCheck, Calendar, Upload, Truck, Cog, Mail, ChevronLeft, ChevronRight, ToolCase } from 'lucide-react';
  ...
  const navigation = [
    ...
    },
    {
      ...
      children: [
        ...
        { name: 'Ingredients', href: '/ingredients', icon: ShoppingCart },
        { name: 'Materials', href: '/materials', icon: ToolCase },
          ...
          <div key={item.name}>
            {item.href ? (
              <Link
                ...
                className={`group flex items-center ${isCollapsed ? 'justify-center px-2' : 'px-2'} py-1.5 text-sm font-bold rounded-md transition-colors ${
                  item.current
              ...
              >
                ...
                {!isCollapsed && (
                  <>
                    ...
                    {item.badge && (
                      <span className="ml-2 inline-flex items-center justify-center px-2 py-1 text-sm font-medium leading-none text-white bg-coral-500 rounded-full">
                        {item.badge}
                      </span>
                    )}
                  </>
              ...
              <div>
                {!isCollapsed && (
                  <div className="px-2 py-1 text-sm font-bold text-gray-500 tracking-wide">
                    {item.name}
                  </div>
                )}
