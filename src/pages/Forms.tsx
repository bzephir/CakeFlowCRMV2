          ...
          <thead>
            <tr>
              {Object.values(FormCategory).map((category, index) => (
                <th
                  key={category}
                  className={`mr-6 px-4 py-6 text-sm text-bold font-medium text-white uppercase tracking-wider bg-gradient-to-r from-coral-400 to-pink-400 rounded-t-lg border-t border-x border-b-0 relative z-10 ${index < Object.values(FormCategory).length - 1 ? 'mr-6' : ''}`}
                >
